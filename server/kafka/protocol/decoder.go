...
func (d *ByteDecoder) Pop() error {
	pd := d.stack[len(d.stack)-1]      //the last element
	d.stack = d.stack[:len(d.stack)-1] //remove the last element
	return pd.Fill(d.off, d.b)         //set the size
}
