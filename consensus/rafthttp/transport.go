...
type Transporter interface {
	...
	SendSnapshot(m snapshot.Message)
...
func (t *Transport) Send(msgs []raftpb.Message) {
	for _, m := range msgs {
		...
		//debug info
		if glog.V(1) {
			glog.Infof("D:ignored message %s (sent to unknown peer %s)", m.Type, to)
		}
	}
}
...
func (t *Transport) SendSnapshot(m snapshot.Message) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (s *nopTransporter) SendSnapshot(m snapshot.Message)     {}
...
type snapTransporter struct {
	nopTransporter
	snapDoneC chan snapshot.Message
	snapDir   string
}

func NewSnapTransporter(snapDir string) (Transporter, <-chan snapshot.Message) {
	ch := make(chan snapshot.Message, 1)
	tr := &snapTransporter{snapDoneC: ch, snapDir: snapDir}
	return tr, ch
}

func (s *snapTransporter) SendSnapshot(m snapshot.Message) {

	//TODO.
	//ss := snapshot.New(s.snapDir)
