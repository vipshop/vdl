...
func createPostRequest(u url.URL, path string, body io.Reader, ct string, urls types.URLs, from, cid types.ID) *http.Request {
	...
	req.Header.Set("X-VDL-RaftGroup-Cluster-ID", cid.String())
...
func checkPostResponse(resp *http.Response, body []byte, req *http.Request, to types.ID) error {
	...
	case http.StatusPreconditionFailed:
		...
		case errIncompatibleVersion.Error():
			glog.Errorf("request sent was ignored by peer %s (server version incompatible)", to)
			return errIncompatibleVersion
		case errClusterIDMismatch.Error():
			glog.Errorf("request sent was ignored (cluster ID mismatch: remote[%s]=%s, local=%s)",
				to, resp.Header.Get("X-VDL-RaftGroup-Cluster-ID"), req.Header.Get("X-VDL-RaftGroup-Cluster-ID"))
...
func minClusterVersion(h http.Header) *semver.Version {
	...
	// backward compatibility with VDL 1.0.0
	if verStr == "" {
		verStr = "1.0.0"
	}
	return semver.Must(semver.NewVersion(verStr))
