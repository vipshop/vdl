...
func (h *pipelineHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("X-VDL-RaftGroup-Cluster-ID", h.cid.String())
	...
	if err != nil {
		glog.Errorf("failed to read raft message (%v)", err)
	...
	if err := m.Unmarshal(b); err != nil {
		glog.Errorf("failed to unmarshal raft message (%v)", err)
...
func newSnapshotHandler(tr Transporter, r Raft, snapshotter *snapshot.Snapshotter, cid types.ID) http.Handler {
...
func (h *snapshotHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if m.Type != raftpb.MsgSnap {
		glog.Errorf("unexpected raft message type %s on snapshot path", m.Type)
		http.Error(w, "wrong raft message type", http.StatusBadRequest)
		return
	}

	glog.Infof("receiving database snapshot [index:%d, from %s] ...", m.Snapshot.Metadata.Index, types.ID(m.From))
	...
	glog.Infof("received and saved database snapshot [index: %d, from: %s] successfully", m.Snapshot.Metadata.Index, types.ID(m.From))
...
func (h *streamHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	default:
		//debug info
		if glog.V(1) {
			glog.Infof("D:ignored unexpected streaming request path %s", r.URL.Path)
		}
...
func checkClusterCompatibilityFromHeader(header http.Header, cid types.ID) error {
	if err := checkVersionCompability(header.Get("X-Server-From"), serverVersion(header), minClusterVersion(header)); err != nil {
		glog.Errorf("request version incompatibility (%v)", err)
		return errIncompatibleVersion
	}
	if gcid := header.Get("X-VDL-RaftGroup-Cluster-ID"); gcid != cid.String() {
		glog.Errorf("request cluster ID mismatch (got %s want %s)", gcid, cid)
