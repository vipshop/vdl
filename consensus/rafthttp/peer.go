...
func startPeer(transport *Transport, urls types.URLs, peerID types.ID, fs *stats.FollowerStats) *peer {
	...
	defer glog.Infof("started peer %s", peerID)
...
func (p *peer) send(m raftpb.Message) {
	...
	default:
		...
		//debug info
		if glog.V(1) {
			glog.Infof("D:dropped %s to %s since %s's sending buffer is full", m.Type, p.id, name)
		}
	}
}

func (p *peer) sendSnap(m snapshot.Message) {
...
func (p *peer) stop() {
	...
	defer glog.Infof("stopped peer %s", p.id)
