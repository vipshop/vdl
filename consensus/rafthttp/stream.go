...
var (
	...
	supportedStream = map[string][]streamType{
		"1.0.0": {streamTypeMsgAppV2, streamTypeMessage},
	}
)
...
func (cw *streamWriter) run() {
	...
	glog.Infof("started streaming with peer %s (writer)", cw.peerID)
	...
	for {
		...
		case <-heartbeatc:
			...
			cw.close()
			glog.Warningf("lost the TCP streaming connection with peer %s (%s writer)", cw.peerID, t)
		...
		case conn := <-cw.connc:
			...
			if closed {
				glog.Warningf("closed an existing TCP streaming connection with peer %s (%s writer)", cw.peerID, t)
			}
			glog.Infof("established a TCP streaming connection with peer %s (%s writer)", cw.peerID, t)
			heartbeatc, msgc = tickc.C, cw.msgc
		case <-cw.stopc:
			if cw.close() {
				glog.Infof("closed the TCP streaming connection with peer %s (%s writer)", cw.peerID, t)
			}
...
func (cr *streamReader) decodeLoop(rc io.ReadCloser, t streamType) error {
	...
	default:
		glog.Fatalf("unhandled stream type %s", t)
	}
	select {
	case <-cr.stopc:
		cr.mu.Unlock()
	...
	for {
		...
		default:
			if cr.status.isActive() {
				glog.Warningf("dropped internal raft message from %s since receiving buffer is full (overloaded network)", types.ID(m.From))
			}
			if glog.V(1) {
				glog.Infof("D:dropped %s from %s since receiving buffer is full", m.Type, types.ID(m.From))
			}
...
func (cr *streamReader) dial(t streamType) (io.ReadCloser, error) {
	...
	req.Header.Set("X-VDL-RaftGroup-Cluster-ID", cr.tr.ClusterID.String())
	...
	case http.StatusPreconditionFailed:
		...
		case errIncompatibleVersion.Error():
			glog.Errorf("request sent was ignored by peer %s (server version incompatible)", cr.peerID)
			return nil, errIncompatibleVersion
		case errClusterIDMismatch.Error():
			glog.Errorf("request sent was ignored (cluster ID mismatch: peer[%s]=%s, local=%s)",
				cr.peerID, resp.Header.Get("X-VDL-RaftGroup-Cluster-ID"), cr.tr.ClusterID)
