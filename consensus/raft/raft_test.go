...
func TestCommit(t *testing.T) {
	...
	for i, tt := range tests {
		...
		conf := newTestConfig(1, []uint64{1}, 5, 1, storage)
		conf.LastHardState = storage.hardState
...
//prevote:https://github.com/coreos/etcd/pull/8346
...
func TestReadOnlyForNewLeader(t *testing.T) {
	...
	for _, c := range nodeConfigs {
		...
		cfg.LastHardState = pb.HardState{Term: 1, Commit: c.committed}
...
func TestBcastBeat(t *testing.T) {
	...
	conf := newTestConfig(1, nil, 10, 1, storage)
	conf.LastHardState = pb.HardState{Term: 1, Commit: offset}
	conf.LastPersistentPeers = []uint64{1, 2, 3}
...
//TODO. remove because VDL haven't implement snapshot currently
...
func votedWithConfig(configFunc func(*Config), vote, term uint64) *raft {
	...
	cfg := newTestConfig(1, []uint64{}, 5, 1, storage)
	cfg.LastHardState = pb.HardState{Vote: vote, Term: term}
	if configFunc != nil {
		configFunc(cfg)
	}
	sm := newRaft(cfg)
