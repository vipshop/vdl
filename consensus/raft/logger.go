...
var (
	...
	defaultLogger = &DefaultLogger{Logger: log.New(os.Stdout, "raft", log.LstdFlags), debug: true}
	discardLogger = &DefaultLogger{Logger: log.New(ioutil.Discard, "", 0)}
	//raftLogger    = Logger(defaultLogger)
	raftLogger = Logger(&Glogger{})
)
...
func (l *Glogger) Debug(v ...interface{}) {
	if glog.V(1) {
		tmp := make([]interface{}, 0, len(v)+1)
		tmp = append(tmp, "D:") //debug tag
		tmp = append(tmp, v...)
		glog.Info(tmp)
	}
}
